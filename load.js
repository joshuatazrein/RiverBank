    ...
    if (this.readyState == 4) {
      if (this.status != 200) {
        ...
        if (newuser) {
          ...
          // user clicked "no"
          const username = prompt('retry username: ');
          const password = prompt('password: ');
          document.cookie = 'username=' + username + '_' + password + 
          '; expires=' + inaweek.toUTCString();
        }
...
if (data.weekdays == 'M') {
  var weekdaysStr = {0:'U', 1:'M', 2:'T', 3:'W', 4:'R', 5:'F', 6:'S'};
  var weekdaysNum = {'U':0, 'M':1, 'T':2, 'W':3, 'R':4, 'F':5, 'S':6};
} else if (data.weekdays == 'Mon') {
  var weekdaysStr = {0:'Sun', 1:'Mon', 2:'Tue', 3:'Wed', 4:'Thu', 5:'Fri', 
  6:'Sat'};
  var weekdaysNum = {'Sun':0, 'Mon':1, 'Tue':2, 'Wed':3, 'Thu':4, 'Fri':5, 
  'Sat':6};
}
