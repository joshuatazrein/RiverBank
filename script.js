...
function dateToHeading(date) {
  ...
  {if (stringToDate($(x).text()) != 'Invalid Date') return true});
...
function search(skiplinks) {
  ...
  for (let search of searches) {
    ...
    children = $('#test').children().toArray();
    for (let child of children) {
      // if it's a match, add to matches
      if ($(child).text().includes(searchtext)) {
        // add to matches
        if (skiplinks == true &&
...
function saveTask() {
  ...
  if (selected.val().charAt(0) == '>') {
    ...
    const list = selected.val().split(' ');
    if (stringToDate(list[0].slice(1)) == 'Invalid Date') {
      alert('invalid date entered');
      selected.val(list.slice(1).join(' '));
    } else {  
      list[0] = '>' + dateToString(stringToDate(list[0].slice(1)));
      selected.val(list.join(' '));
    }
  }
...
function context(e) {
  ...
  options = {
    ...
    '#context-newTask': [['SPAN'], ['in']],
    '#context-newSubtask': [['SPAN'], ['in']],
    '#context-newTaskBlank': [['P'], ['in']],
...
$('#username').val(document.cookie.split(';')[0].split('=')[1]);
$(document).on('beforeunload', finalsave);
